...
inc_by_year <- cases %>%
  ...
  mutate(incidence = round(annual_cases * 100000 / annual_pop, digits = 1))
...
inc_by_year_and_dis <- cases %>%
  ...
  mutate(incidence = round(annual_cases * 100000 / pop, digits = 1)) %>%
  ungroup() %>%
  mutate(disease = as.character(disease))
inc_by_year_and_dis <- select(inc_by_year_and_dis, date, disease, incidence)
  
# convert to grouped time series 
wide_inc_by_year_and_dis <- dcast(inc_by_year_and_dis, date ~ disease)
ts_by_year_and_dis <- ts(subset(wide_inc_by_year_and_dis, select=-date), 
                           frequency = 4, 
                           start = 2001)
# only plot disease level, not total
to_plot <- aggts(gts(ts_by_year_and_dis), levels=c(1))
plot(to_plot, type="b", main="Quarterly Incidences")
...
To give an example, in the population data the age was included in the form "6 year olds" or "over 90 year olds". I had to extract the numeric value and, because the disease data grouped everyone over 80 together, had to sum all ages above 80. Additionally, I had to sum the values for the various states of Germany to obtain population numbers for the regions.

In terms of unusual distributions, when I look at the quarterly incidences, there is a spike in EHEC diseased and E.Coli drops to 0 in 2015.
...
mean_inc_by_age_dis <- cases %>%
  ...
  mutate(incidence = round(annual_cases * 100000 / mean_pop,  digits = 1))
...
I think the difference between campylobacter, norovirus and salmonella on the one hand and rotavirus on the other are much clearer if plotted this way.
...
This shows that only 3 to 5 diseases seem to be responsible for the majority of cases, which is also something I wanted to investigate earlier in the univariate section. Here is the variability the monthly incidences of each disease in Germany for all ages.
...
monthly_inc <- monthly_inc %>%
  ...
  # calculate incidence
  left_join(annual_pop, by = c("pop_year" = "year")) %>%
  mutate(incidence = round(month_cases * 100000 / pop, digits = 1))
...
Influenza has a low mean but quite a few outliers, up to 109.  

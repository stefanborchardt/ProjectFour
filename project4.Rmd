...
After the exploration of age/ region/ disease is eshausted, I will have a look at the seasonality again. 
...
```{r echo=FALSE, fig.width=10, fig.height=8, Multivariate_Plots3}

inc_noro_age_dis <- cases %>%
  filter(disease == "noro") %>%
  # interested in year and week number only
  mutate(year = year(date)) %>%
  dplyr::select( - date) %>%
  # sum over regions
  group_by(year, week, pop_year, age) %>%
  ...
  # discard error-prone weeks
  filter(week > 1 & week < 52) %>%
  dplyr::select(year, week, age, incidence)

pl <- function (age) {
  data <- inc_noro_age_dis[inc_noro_age_dis$age == age, ]
  ggplot(data, aes(x = year, y = week, fill=incidence)) +
    geom_raster() +
    scale_fill_gradientn(colours = heat_hcl(20)) +
    ggtitle(age)
...
While the seasonal pattern is similar within the groups, it looks different between the groups. I will consider that in the final version of the seasonality plot.

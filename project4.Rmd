...
# data for gastroenteritis of one health insurance (AOK)  of 2005, the only available
aok_inc <- 6224.3
...
points(x = 2005, y = aok_inc, type="b", pch=19, col="blue")
legend("topright", c("AOK / sick leaves non-infect. gastroent.","RKI / infectious gastroenteritis only"), fill=c("blue", "black") )  

```

With the help of a doctor I found out that at least 80% of the cases have an infectious cause, which means I should see a yearly incidence of at least 30,000 in the RKI's data. The plotted values are much lower. I checked this against the yearly values through the institute's interface and noticed even much lower values (240-560) there. 
...
First, I revisit the disease distribution by age, only this time incidences are not counted but summed up. I experiment on how to indicate the different bin widths

```{r echo=FALSE, fig.width=10, fig.height=5, warning=FALSE, Bivariate_Plots}
# group and sum
sum_inc_by_age_and_dis <- summarize(group_by(long_inc, age, disease), sum_inc = sum(incidence))
# add the widths of the age groups
sum_inc_by_age_and_dis$wdth <- c(rep(rep(1, 9), 6), rep(rep(5, 9), 4), rep(rep(10, 9), 6))

ggplot(sum_inc_by_age_and_dis, aes(
  x = age, y = sum_inc, fill = disease, width = wdth / 12)) +
  geom_bar(stat = "identity", position = "identity", colour = "black") +
  guides(fill = guide_legend(reverse = TRUE)) 

sum_inc_by_age_and_dis$wdth <- NULL

```

Rather confusing. I try to convert the factor age group to a numeric value.

```{r echo=FALSE, fig.width=10, fig.height=7, warning=FALSE, Bivariate_Plots2}

#convert factor age group to numeric floor of age
sum_inc_by_age_and_dis$age <- as.character(sum_inc_by_age_and_dis$age)
sum_inc_by_age_and_dis$age <- gsub(pattern = "A", replacement = "",
                                   x = sum_inc_by_age_and_dis$age)
sum_inc_by_age_and_dis$age <- as.numeric(sum_inc_by_age_and_dis$age)

ggplot(sum_inc_by_age_and_dis, aes(x = age, y = sum_inc, fill = disease)) +
...
Converting the factor age group to a numeric value worked. I had not expected that the differences are that big. Again, I have to consider that the data contains reported cases, maybe adults just do not see a doctor when they have the syptoms. 

Another aspect is that I assume that the incidence is calculated on the population total, and not by the population of the age group. I expect the differences to be even more extreme than what I see here.
...
pop <- select(population, - ends_with("2"))

# sum by gender and region
pop <- mutate(pop, mal_n = Bremen + Hamburg + Mecklenburg.Vorpommern + 
                                 Niedersachsen + Schleswig.Holstein)
pop <- mutate(pop, fem_n = Bremen.1 + Hamburg.1 + Mecklenburg.Vorpommern.1 +
                                 Niedersachsen.1 + Schleswig.Holstein.1)
pop <- mutate(pop, mal_e = Berlin + Brandenburg + Sachsen + Sachsen.Anhalt +
                                 Thüringen)
pop <- mutate(pop, fem_e = Berlin.1 + Brandenburg.1 + Sachsen.1 + Sachsen.Anhalt.1 +
                                 Thüringen.1)
pop <- mutate(pop, mal_s = Baden.Württemberg + Bayern)
pop <- mutate(pop, fem_s = Baden.Württemberg.1 + Bayern.1)
pop <- mutate(pop, mal_w = Hessen + Nordrhein.Westfalen + Rheinland.Pfalz + Saarland)
pop <- mutate(pop, fem_w = Hessen.1 + Nordrhein.Westfalen.1 + Rheinland.Pfalz.1 +
                                 Saarland.1)
pop <- select(pop, X, X.1, contains("_"))

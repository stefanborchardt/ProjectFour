...
calc_inc <- function (case_count, pop_count) {
  ...
  return(round(case_count * 100000 / pop_count, digits = 1))
} 
...
Region East has higher incidences for all diseases in almost every single month. Are doctors there more eager to report or is this caused by the age structure?

```{r echo=FALSE, fig.width=10, fig.height=7, Bivariate_Plots7}
...
ggplot(pop_east_rest, aes(x = age, size = pop_prop, y = region, colour=diff)) +
...
The size of the dots shows what proportion the different ages have within the east and the rest, while color indicates how big the difference between the regions is. The East has relatively fewer people around 20 and 50, and more people 60+. 

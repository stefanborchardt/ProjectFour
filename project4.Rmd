...
aok_inc <- as.data.frame(t(aok_inc))
...
```{r eval = FALSE, echo=FALSE, message=FALSE, Load_the_Data3}
# Load the Data
####################################################################################
#########  switch to eval = TRUE if you want to read 'raw' files again #############
#########  otherwise see next chunk                                    #############
...
# only need the year, using population of 31/12/2000 for the years 2000 and 2001
...
# avoid int overflows when summing
population$pop_count <- as.numeric(population$pop_count)
pop_levels <- levels(population$region)
...
# add population counts on composite key
cases <- left_join(cases, population, 
                        by = c("pop_year" = "year", "age" = "age", "region" = "region"))
cases <- subset(cases, select = - pop_year)
cases <- arrange(cases, date)

# calculate incidence per 100,000 and round to .1
cases$incidence <- round(cases$case_count * 100000 / cases$pop_count, digits = 1)

write.csv(cases, file="cases.csv", row.names=F)

# wide format necessary for the time series plot. 
wide_inc <- dcast(cases, date ~ disease + age + region)
...
to_group <- select(cases, date, disease, age, cases, region, pop_count)

cases <- cases %>% 
  group_by(age, disease) %>% 
  mutate(sum_cases = sum(cases)) %>%
  group_by(age, disease) %>% 
  mutate(sum_pop = sum(pop_count))
cases$age_inc <- cases$sum_cases * 100000 / cases$sum_pop

sum_by_age_and_dis <- summarize(group_by(cases, age, disease), sum_cases = sum(cases), sum_pop = sum(pop_count)/36)
pop_by_age <- summarise(group_by(population, age))
sum_by_age_and_dis$incidence <- round(sum_by_age_and_dis$cases * 100000 / sum_by_age_and_dis$pop_count)

theme_set(theme_few())

ggplot(sum_by_age_and_dis, aes(x = age, y = incidence, fill = disease)) +
